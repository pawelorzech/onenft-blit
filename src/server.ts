...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (!today) {
      ...
      if (path === "/today.svg") return svg(blitFor(dayOne.epoch).svg, false);
    ...
    if (path === "/today.svg") return svg(blitFor(today.epoch).svg, false);
    ...
    if (path === "/wordmark.svg") return svg(wordmarkSvg(blitFor(today.epoch)), false);
    ...
    if (m) {
      ...
      if (m[2] === ".svg") return svg(blitFor(d.epoch).svg, n < today.n);
    ...
    if (pv) {
      ...
      return svg(blitFor(d.epoch).svg, false);
    }
...
console.log(`blit.onenft.click on :${PORT}`);
