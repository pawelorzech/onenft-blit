/**
 * PNG cards for link previews and feeds. 1200×630: the blit on the left,
...
export function cardSvg(day: Day): string {
  ...
  const t = k.traits;
  const line = `${t.composition} by ${t.compositionArtist}, colors of ${t.palette} by ${t.paletteArtist}`;
...
<text x="662" y="560" font-family="Syne" font-weight="800" font-size="${fitSize("blit.onenft.click", "Syne", 44, 800)}" fill="${fg}">blit.onenft.click</text>
</svg>`;
}
...
/** The blit itself as a square PNG, for the download links that work without JavaScript. Pixel art, so no smoothing. */
...
export function squarePng(day: Day, immutable: boolean): Uint8Array {
  return remembered(`square${day.n}`, immutable, squares, day.n, () => new Resvg(blitFor(day.epoch).svg, { fitTo: { mode: "width", value: SQUARE_PX }, imageRendering: 1 }).render().asPng());
}
