...
import { SITE, REPO, PARENT, BLITMAP, layout, topBar, label, shortAddr, isAuthor, explorer, opensea, openseaCollection, chainName, num, plural, stripSize, esc, siblingLink, type Names, NO_NAMES } from "./site.ts";
...
export function holderPage(who: Address, handle: string, today: Day, chain: ChainState, names: Names = NO_NAMES): string {
  ...
  const cards = mine.map((n) => {
    const kk = blitFor(dayByNumber(n)!.epoch);
    return `<a href="/day/${n}"><img src="/day/${n}.svg" alt="" loading="lazy"><div class="cap">day ${n}, ${esc(kk.traits.composition)} in ${esc(kk.traits.palette)}</div></a>`;
  });
  const body = `<main class="wide">
${topBar()}
...
${cards.length ? `<div class="strip">${cards.join("")}</div>` : `<p>No days here yet. <a href="/">Today's blit</a> may still be free.</p>`}
