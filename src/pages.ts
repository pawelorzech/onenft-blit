...
import { SITE, REPO, PARENT, FILE_PREFIX, BLITMAP, layout, topBar, footer, label, shortAddr, isAuthor, explorer, opensea, openseaCollection, chainName, num, plural, stripSize, esc, siblingLink, afterMidnight, traitList, whoBlock, sizePicker, downloadBar, connectScript, downloadScript, nameHeading, staleNote, dayState, type Names, NO_NAMES } from "./site.ts";
...
${topBar("Explore")}
${staleNote(status)}
<div><h1 class="syne">Every day so far</h1><p class="lead" style="margin-top:8px">${today.n} ${plural(today.n, "day", "days")} remixed${chain ? `, ${taken} claimed, ${gaps} ${plural(gaps, "gap", "gaps")}` : ""}. Hatched days are gaps: the day ended without a claim, and it can no longer be minted. Dimmed days have not happened yet.</p></div>
${months.join("\n")}
...
function seenTable(rows: { id: string; label: string; artist: string; seen: Seen | undefined; odds: number }[], chain: ChainState | null, kind: string): string {
  ...
  return `<div class="scroll"><table class="tr"><thead><tr><th>${kind}</th><th style="text-align:right">odds</th><th style="text-align:right">so far</th>${chain ? `<th style="text-align:right">claimed</th>` : ""}<th>days</th></tr></thead><tbody>${tr.join("")}</tbody></table></div>`;
}
...
export function traitsPage(today: Day, chain: ChainState | null = null): string {
  ...
  const body = `<main class="wide" id="main">
${topBar("Traits")}
<div><h1 class="syne">Traits</h1><p class="lead" style="margin-top:8px">Every day pairs one composition with one palette out of the same 100 originals, so each original comes up as a composition once in 100 days and as a palette once in 100 days, in the long run. So far counts the ${today.n} ${plural(today.n, "day", "days")} remixed to date${chain ? ", claimed counts only claimed days" : ""}. The originals themselves are on Ethereum, contract <a href="https://etherscan.io/address/${BLITMAP}">${shortAddr(BLITMAP)}</a>.</p></div>
...
${topBar("Your wallet")}
${staleNote(status)}
<div><h1 class="syne">Your days</h1><p class="lead" style="margin-top:8px">Connect a wallet or type an address, and this page lists every blit it holds, each one ready to save as SVG, PNG or JPEG.</p></div>
...
<p>Any day as SVG at <code>/day/N.svg</code>, as a 1024 pixel PNG at <code>/day/N-1024.png</code>, and as a 1200 by 630 link card at <code>/day/N.png</code>. Today: <a href="/today.svg" download="blit-today.svg">SVG</a>, <a href="/today.png">card</a>. The SVG is the same file the contract holds. Render it with <code>image-rendering: pixelated</code> so the pixels stay square.</p>
