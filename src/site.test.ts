...
test("home lists every earlier day, today not among the rows", () => {
  ...
  expect(h).toContain("<title>Day 7 | blit.onenft.click</title>");
});
...
test("how page explains the traits and points at the spec", () => {
  ...
  for (const w of ["composition", "palette", "affinity", "perm(", "9,900", "/spec.json", "CC0"]) expect(f).toContain(w);
});
...
test("feed lists days newest first with a PNG enclosure", () => {
  ...
  expect(x).toContain('enclosure url="https://blit.onenft.click/day/2.png"');
  expect(x).toContain("Nobody came");
});

test("every page carries og:image and the feed link", () => {
  const t = dayByNumber(2)!;
  expect(homePage(t, t.startsAt)).toContain('og:image" content="https://blit.onenft.click/day/2.png"');
...
test("OpenSea collection link appears with a contract, never without", () => {
  ...
  expect(homePage(t, t.startsAt, { ...fakeChain(5, {}), chainId: 8453 })).toContain("opensea.io/collection/blit-onenft-click");
});
