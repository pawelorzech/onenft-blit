...
test("traits: every original in both tables, the affinities seen, the sibling count", () => {
  const h = traitsPage(dayByNumber(40)!, fakeChain(40, { 1: A }));
  for (const w of ["Genesis", "Rose", "Boombox", "by dom", "1%", 'id="c13"', 'id="p88"', 'id="affinity"', 'id="sibling"', "1,600", "9,900", "Water II, Earth I"]) expect(h).toContain(w);
  expect((h.match(/<tr id="c\d+"/g) ?? []).length).toBe(100);
  expect((h.match(/<tr id="p\d+"/g) ?? []).length).toBe(100);
});
...
test("assets page states CC0 and the embed snippet", () => {
  ...
  expect(h).toContain("&lt;iframe src=&quot;https://blit.onenft.click/embed&quot;");
...
test("day JSON carries traits, owner and the claim", () => {
  ...
  expect(j.traits.composition).toBe("Jupiter");
  expect(j.traits.palette).toBe("Fishy");
  expect(j.pair).toEqual({ composition: 13, palette: 88, index: 13 * 99 + 87, cycle: 0 });
  expect(j.data.length).toBe(2 + 268 * 2);
...
test("days JSON has one entry per day and the spec has the tables", () => {
  ...
  const s = specJson();
  expect(s.originals.length).toBe(100);
  expect(s.siblings.length).toBe(1600);
  expect(s.pairs).toBe(9900);
...
test("holder page: one row per day with a download bar, the size picker once, the way to the hub", () => {
  ...
  expect(h).toContain('download="blit-day-1.svg"');
