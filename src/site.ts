 ...
 * It takes today's four colors from the remix, so it looks different every
 ...
 */
import { blitFor, type Palette, type Blit } from "./blit.ts";
...
export const SITE = "blit.onenft.click";
export const REPO = "https://github.com/pawelorzech/onenft-blit";
export const PARENT = "onenft.click";
/** The Ethereum contract the originals come from. */
export const BLITMAP = "0x8d04a8c79ceb0889bdd12acdf3fa9d207ed3ff63";
...
export function openseaCollection(chain: ChainState): string {
  return chain.chainId === 8453 ? "https://opensea.io/collection/blit-onenft-click" : `https://testnets.opensea.io/assets/base_sepolia/${chain.address}`;
}
...
<title>${title}</title>
<meta name="description" content="One Blitmap remix a day, computed on chain from the clock of the Base chain: the composition of one original in the palette of another.">
...
<meta property="og:description" content="One Blitmap remix a day, computed on chain from the clock of the Base chain.">
...
/** Today's four colors as swatches. */
export function swatches(k: Blit): string {
  return `<div class="tiles" aria-hidden="true">${k.palette.colors.map((c) => `<i style="background:${c}" title="${c}"></i>`).join("")}</div>`;
}
/** OpenSea page of the Ethereum Blitmap that already holds a pair. */
export function siblingLink(sibling: string): string {
  const m = sibling.match(/#(\d+)/);
  return m ? `<a href="https://opensea.io/assets/ethereum/${BLITMAP}/${m[1]}">${sibling}</a>` : sibling;
}
...
/** The six traits as a definition list, the way they sit in the token metadata. */
export function traitList(k: Blit): string {
  const t = k.traits;
  const rows: [string, string][] = [
    ["composition", `<a href="/traits#c${k.a}">${esc(t.composition)}</a> <span class="small">by ${esc(t.compositionArtist)}</span>`],
    ["palette", `<a href="/traits#p${k.b}">${esc(t.palette)}</a> <span class="small">by ${esc(t.paletteArtist)}</span>`],
    ["colors", k.palette.colors.map((c) => `<span style="display:inline-block;width:.8em;height:.8em;background:${c};box-shadow:0 0 0 1px var(--line);vertical-align:-1px;margin-right:6px"></span>`).join("")],
    ["affinity", `<a href="/traits#affinity">${t.affinity}</a>`],
    ["on Ethereum", t.sibling === "none" ? `<a href="/traits#sibling">not yet, this pair is new</a>` : `as ${siblingLink(t.sibling)}`],
  ];
...
export function homePage(today: Day, now: bigint, chain: ChainState | null = null, names: Names = NO_NAMES): string {
  ...
  for (let n = today.n - 1; n >= Math.max(1, today.n - 60); n--) {
    ...
    const who = owner ? (isAuthor(chain!, owner) ? "the author's" : `taken by ${label(owner, names)}`) : `${esc(kk.traits.composition)} in ${esc(kk.traits.palette)}`;
  ...
  let cta = `<a class="cta syne" href="/day/${today.n}.svg" download="blit-day-${today.n}.svg">Download today's blit</a>
...
<p class="small">The next one pairs tomorrow. ${fmtLeft(left)} left.${badge}</p>`;
    } else if (authorDay) {
...
<p class="small">Written into the contract from day one, up to day 1000. Tomorrow is yours again.${badge}</p>`;
      ...
      cta = `<button class="cta syne" id="mint">Claim today's blit</button>
...
${crumb()}
<h1 class="syne">One<br>remix<br>a day</h1>
<p class="lead">Every day at midnight UTC the contract takes the composition of one Blitmap original and the palette of another. 100 originals by 17 artists, released CC0 in 2021; 9,900 pairs; one a day, each pair once in 27 years. The clock does the pairing.</p>
<hr>
<div><div class="big syne">${today.n}</div><div class="small">${plural(today.n, "day remixed", "days remixed")}</div></div>
...
<p class="lead" style="max-width:330px">The contract paired this blit at midnight UTC, ${dateOf(today.epoch)}. It pairs the next one in <span data-left="${left}">${fmtLeft(left)}</span>.</p>
<hr>
${traitList(k)}
...
export function dayPage(d: Day, today: Day, chain: ChainState | null = null, names: Names = NO_NAMES): string {
  ...
  const snippet = esc(`<a href="${url}"><img src="https://${SITE}/day/${d.n}.svg" width="256" height="256" alt="Day ${d.n} of blit.onenft.click"></a>`);
...
<nav class="nav small">${chain && chain.owners.has(d.n) ? `<a href="${opensea(chain, d.n)}">OpenSea</a><a href="${explorer(chain.chainId)}/nft/${chain.address}/${d.n}">Basescan</a>` : ""}<a href="/day/${d.n}.svg" download="blit-day-${d.n}.svg">SVG</a><a href="/day/${d.n}.png">PNG</a></nav>
${share}
...
<p><strong>The pair</strong> for day n is number perm(n - 1) out of the 9,900 possible pairs: 100 compositions times 99 other palettes. perm is a fixed shuffle of 0 to 9,899, so every pair appears exactly once in 9,900 days, about 27 years. After that the shuffle changes key and the next 9,900 days run in a new order. An original is never paired with itself; you can see the originals themselves on Ethereum.</p>
<p><strong>The remix</strong> is 268 bytes: the first 12 (four RGB colors) from the palette original, the other 256 (1,024 pixels, two bits each) from the composition original. The same rule Blitmap used for its siblings, so when a pair already exists there the page says so and links to it.</p>
${swatches(k)}
<p><strong>The image</strong> is one SVG: a rect in the most used color, then one rect per horizontal run of any other color. A few kilobytes. The contract returns it as a <code>data:</code> URI, with no server in between. This page takes its colors from today's four, so it looks different every day. Today: ${esc(k.traits.composition)} in the colors of ${esc(k.traits.palette)}. <a href="/traits">See every original and how often it has come up.</a></p>
<p><strong>The traits</strong> in the token are the composition and its artist, the palette and its artist, Blitmap's affinity (Fire, Earth or Water from the first three colors) and the Ethereum sibling when one exists.</p>
<h2 class="syne">Build it yourself</h2>
<p>The same day number gives the same blit every time, ten years from now and with this page switched off. The generator is in <a href="${REPO}">the repository</a>, in TypeScript and in Solidity, with a test that keeps the two byte for byte equal. The shuffle is below; the originals, their names and the sibling list are in <a href="/spec.json">spec.json</a>.</p>
...
export function feedXml(today: Day, chain: ChainState | null): string {
  ...
  for (let n = today.n; n >= Math.max(1, today.n - 30); n--) {
    ...
    items.push(`<item><title>Day ${n}</title><link>https://${SITE}/day/${n}</link><guid isPermaLink="true">https://${SITE}/day/${n}</guid><pubDate>${date}</pubDate><description>&lt;img src="https://${SITE}/day/${n}.png" alt=""&gt;&lt;p&gt;Day ${n}, ${dateOf(d.epoch)} UTC: ${esc(k.traits.composition)} in the colors of ${esc(k.traits.palette)}.${state}&lt;/p&gt;</description><enclosure url="https://${SITE}/day/${n}.png" type="image/png" length="0"/></item>`);
  }
  return `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0"><channel><title>blit.onenft.click</title><link>https://${SITE}/</link><description>One Blitmap remix a day, computed on chain from the clock of the Base chain.</description><language>en</language>
${items.join("\n")}
</channel></rss>`;
}
