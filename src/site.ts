...
export function homePage(today: Day, now: bigint, chain: ChainState | null = null, names: Names = NO_NAMES, status: ChainStatus | null = null): string {
  ...
  for (let n = today.n - 1; n >= Math.max(1, today.n - 60); n--) {
    ...
    const who = owner ? (isAuthor(chain!, owner) ? "the author's" : `held by ${label(owner, names)}`) : st === "unknown" ? "status unavailable" : `${esc(kk.traits.composition)} in ${esc(kk.traits.palette)}`;
  ...
  let cta = "";
  if (!chain) {
    cta = `<a class="cta syne" href="/day/${today.n}-1024.png" download="${FILE_PREFIX}-day-${today.n}-1024.png">Download today's blit</a>
...
<p class="small">0 ETH mint fee. You pay network gas. ${fmtLeft(left)} left today.${badge}</p>`;
  ...
  const counts = `<div class="counts syne"><div><b>${today.n}</b><span class="small">${plural(today.n, "day remixed", "days remixed")}</span></div>${chain
...
<p class="lead">One Blitmap remix a day: the composition of one CC0 original in the palette of another, paired by a contract on Base from the day number.</p>
</div></aside>
<main id="main">
${staleNote(status)}
${NEW_DAY}
...
<p class="lead" style="max-width:330px">Day ${today.n}, ${dateOf(today.epoch)} UTC. The day number alone determines this pair. The next day starts in <span data-left="${left}">${fmtLeft(left)}</span>.</p>
<div class="ctas">
${cta}
</div>
${counts}
...
export function feedXml(today: Day, chain: ChainState | null): string {
  ...
  for (let n = today.n; n >= Math.max(1, today.n - 30); n--) {
    ...
    items.push(`<item><title>Day ${n}</title><link>https://${SITE}/day/${n}</link><guid isPermaLink="true">https://${SITE}/day/${n}</guid><pubDate>${date}</pubDate><description>&lt;img src="https://${SITE}/day/${n}.png" alt=""&gt;&lt;p&gt;Day ${n}, ${dateOf(d.epoch)} UTC: ${esc(k.traits.composition)} in the colors of ${esc(k.traits.palette)}.${esc(state)}&lt;/p&gt;</description><enclosure url="https://${SITE}/day/${n}.png" type="image/png" length="0"/></item>`);
  }
  return `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0"><channel><title>${SITE}</title><link>https://${SITE}/</link><description>One Blitmap remix a day, computed on chain from the clock of the Base chain.</description><language>en</language>
${items.join("\n")}
</channel></rss>`;
}
...
<p><strong>The contract is the product.</strong> A minted token keeps its image; the author can change the drawing rules for days not yet claimed, never for a claimed day. The contract has no pause and no way to take a token back. Once a day has passed, an unclaimed day stays empty.</p>
