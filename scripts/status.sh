...
NET="${1:-mainnet}"
case "$NET" in
  mainnet) RPC=https://mainnet.base.org; SITE=https://blit.onenft.click;;
  sepolia) RPC=https://sepolia.base.org; SITE=https://blit-test.onenft.click;;
  *) echo "mainnet|sepolia"; exit 1;;
esac
D="$HOME/.config/onenft-blit/deploy-$NET.json"
NFT=$(jq -r .OneNFT "$D")
