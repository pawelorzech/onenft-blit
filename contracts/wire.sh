...
# Reads ~/.config/onenft-blit/deploy-<net>.json and the app uuids from ~/.config/onenft-blit/coolify.json,, sets CONTRACT_ADDRESS/CHAIN_ID/START_EPOCH/BASE_RPC_URL in Coolify, redeploys.
set -euo pipefail
NET="${1:?sepolia|mainnet}"
case "$NET" in
  sepolia) APP=$(jq -r .test "$HOME/.config/onenft-blit/coolify.json"); RPC=https://sepolia.base.org;;
  mainnet) APP=$(jq -r .prod "$HOME/.config/onenft-blit/coolify.json"); RPC=https://mainnet.base.org;;
  *) echo "sepolia|mainnet"; exit 1;;
esac
D="$HOME/.config/onenft-blit/deploy-$NET.json"; [ -f "$D" ] || { echo "no $D"; exit 1; }
