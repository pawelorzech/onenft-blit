...
contract OneNFTTest is Test {
    ...
    BlitRenderer renderer;
    OneNFT nft;

    function setUp() public {
        vm.warp(START * EB - 100);
        string memory d = vm.readFile("test/fixtures/blit_data.json");
        renderer = new BlitRenderer(
            START,
            DataStore.write(vm.parseJsonBytes(d, "$.originalsA")),
            DataStore.write(vm.parseJsonBytes(d, "$.originalsB")),
            DataStore.write(vm.parseJsonBytes(d, "$.meta")),
            DataStore.write(vm.parseJsonBytes(d, "$.siblings"))
        );
        nft = new OneNFT("blit.onenft.click", "BLITDAY", START, author, address(renderer));
    }
