...
LOG="/tmp/onenft-blit-deploy-$NET.log"
START_EPOCH=$START_EPOCH AUTHOR=$AUTHOR forge script script/Deploy.s.sol --rpc-url "$RPC" --broadcast --private-key "$PK" \
  --verify --verifier sourcify 2>&1 | tee "$LOG" | grep -E "originals|meta|siblings|BlitRenderer|OneNFT|startEpoch|verif|Error" || true
unset PK
get() { grep -E "^\s*$1 " "$LOG" | head -1 | awk '{print $2}'; }
mkdir -p "$HOME/.config/onenft-blit"
jq -n --arg net "$NET" --argjson chain "$CHAIN" --arg nft "$(get OneNFT)" --arg ren "$(get BlitRenderer)" --arg a "$(get originalsA)" --arg b "$(get originalsB)" --arg m "$(get meta)" --arg s "$(get siblings)" \
  --argjson start "$START_EPOCH" --arg author "$AUTHOR" --arg deployer "$DEPLOYER" --arg at "$(date -u +%FT%TZ)" \
  '{network:$net,chainId:$chain,OneNFT:$nft,BlitRenderer:$ren,originalsA:$a,originalsB:$b,meta:$m,siblings:$s,startEpoch:$start,author:$author,deployer:$deployer,at:$at}' > "$HOME/.config/onenft-blit/deploy-$NET.json"
